...
class ResNet50SD(nn.Module):
    ...
    def __init__(
        ...
        super(ResNet50SD, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        ...
        self.fc1 = nn.Linear(512 * block.expansion, n_class)
        self.fc2 = nn.Linear(512 * block.expansion, n_class)
        self.fc3 = nn.Linear(512 * block.expansion, n_class)
        self.fc4 = nn.Linear(512 * block.expansion, n_class)

        # rot_classifier for Remix Match
        self.is_remix = is_remix
        if is_remix:
            self.rot_classifier1 = nn.Linear(2048, 4)
            self.rot_classifier2 = nn.Linear(2048, 4)
            self.rot_classifier3 = nn.Linear(2048, 4)
            self.rot_classifier4 = nn.Linear(2048, 4)
...
class build_ResNet50SD:
    def __init__(self, is_remix=False):
        self.is_remix = is_remix

    def build(self, num_classes):
        return ResNet50SD(n_class=num_classes, is_remix=self.is_remix)


if __name__ == '__main__':
    a = torch.rand(16, 3, 224, 224)
    net = ResNet50SD(is_remix=True)
    x,y = net(a)
    print(x.shape)
    print(y.shape)
